...
public class JsonIslemleri {
            ...
            for (int i = 0; i < ogrenciler.size(); i++) {
                ...
                if (!ogr.getDersNotlari().isEmpty()) {
                    ...
                    for (Map.Entry<String, Double> entry : ogr.getDersNotlari().entrySet()) {
                        ...
                        if (c++ < ogr.getDersNotlari().size() - 1) writer.write(", ");
                    }
                }
                writer.write("}");
                if (i < ogrenciler.size() - 1) writer.write(",");
                writer.newLine();
            }
            writer.write("]");
            System.out.println("Kayıt güncellendi: ogrenciler.json");
        } catch (IOException e) {
    ...
    public static List<Ogrenci> ogrencileriYukle() {
        // Eski kodunuzun aynısı, SADECE class kapanış parantezinden önce dursun
        // ...
        return new ArrayList<>(); // (Sizin kodunuzda dolu dönüyor)
    }
}